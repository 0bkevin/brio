...
export function explainConnectionError(reason: unknown): ConnectionIssue {
  ...
  if (message.includes('unauthorized') || message.includes('401') || message.includes('token')) {
    return {
      ...
      detail: 'The details may no longer match Brio Companion on your computer.',
      checklist: ['Run `brio companion pair` again', 'Scan or paste the current connection details'],
    };
  }
